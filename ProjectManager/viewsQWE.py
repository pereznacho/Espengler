...
def generate_report(request, project_id):
    if request.method == 'POST':
        ...
        if cover.imagen_proveedor:
            # Crear un nuevo párrafo sin alineación específica
            paragraph = doc.add_paragraph()
            paragraph.alignment = WD_ALIGN_PARAGRAPH.LEFT  # Asegurar que el párrafo esté alineado a la izquierda

            # Añadir la imagen con el ancho completo de la página
            run = paragraph.add_run()
            run.add_picture(cover.imagen_proveedor.path, width=Inches(8.5))  # Ancho de 8.5 pulgadas para cubrir todo el ancho de la página

            # Ajustar el espaciado entre la imagen y el texto
            paragraph_format = paragraph.paragraph_format
            paragraph_format.space_after = 0  # Eliminar el espacio después de la imagen

            # Eliminar cualquier espacio extra después de la imagen
            doc.add_paragraph("")
            ...
            if cover.customer_header_image:  # Asegúrate de que este campo existe en tu modelo
                header = doc.sections[0].header
...
def graph_view(request):
    nodes = []
    links = []
    targets = Target.objects.filter(owned=True)

    for target in targets:
        nodes.append({"id": target.ip_address, "label": target.ip_address})
        if target.jumped_from:
            links.append({"source": target.jumped_from.ip_address, "target": target.ip_address})
