...
function create(type) {
  return d3__namespace.select(document.createElementNS("http://www.w3.org/2000/svg", type));
}
...
function Node(data) {
  ...
  let nodeShape = exists ? d3__namespace.select(this) : create("g");
  nodeShape.classed("gly-node", true).attr("data-id", data.id);
  ...
  renderTemplate(data.shape.template, data, {
    ...
    nodeShape.append(() => d3__namespace.select(render).select("[data-object=shape]").node()).attr("data-object", "shape");
    if (data.forceSimulation) {
...
function dragNode() {
  return d3__namespace.drag().on("start", (e, d) => dragStart.bind(this)(e, d)).on("drag", (e, d) => dragMove.bind(this)(e, d)).on("end", (e, d) => dragEnd.bind(this)(e, d));
}
...
function calculateIntersectionDistance(sourceNode, targetNode, link, reversed = false) {
  var _a, _b;
  const linePath = d3__namespace.select("[data-id='" + linkID(link) + "']").select("[data-object='link-line-full']").node();
...
function linkForce(distance) {
  const force = d3__namespace.forceLink().id((d) => d.id).strength((d) => {
...
function xForce() {
  const force = d3__namespace.forceX().x((d) => position(d).x).strength((d) => strength(d));
  return force;
}
function yForce() {
  const force = d3__namespace.forceY().y((d) => position(d).y).strength((d) => strength(d));
  return force;
}
function gravity(envGravity) {
  const force = d3__namespace.forceManyBody().strength((d) => {
    if (d.gravity)
      return d.gravity;
    return envGravity;
  });
...
function shapeCollide(alpha) {
  ...
  var quadtree = d3__namespace.quadtree().x((d) => d.x).y((d) => d.y).addAll(nodes);
  nodes.forEach((d) => {
    var _a;
    d.x = d.x || 0;
    d.y = d.y || 0;
...
function createZoom() {
  const zoom = d3__namespace.zoom().extent([
    [0, 0],
  ...
  this.svgSelection.call(zoom).call(
    zoom.transform,
    d3__namespace.zoomIdentity.translate(this.worldTransform.x, this.worldTransform.y).scale(this.worldTransform.k)
  );
...
function nodesBoundary(nodes) {
  const bounds = d3__namespace.extent(nodes, (n) => n.x);
  const x = d3__namespace.extent(nodes, (n) => n.y);
  const width = (bounds[1] ?? 0) - (bounds[0] ?? 0);
  const height = (x[1] ?? 0) - (x[0] ?? 0);
  const largestNodeSize = d3__namespace.max(nodes.map((n) => {
    var _a;
...
function transform(t, onZoom2, duration) {
  if (isNaN(t.x) || isNaN(t.y) || isNaN(t.k))
    return;
  this.selectionGroups.world.transition().duration(duration ?? 1e3).ease(d3__namespace.easePolyInOut).attr("transform", `translate(${t.x},${t.y}) scale(${t.k})`).on("end", () => {
    onZoom2(t);
  });
}
...
class ForceSimulation {
  constructor(svgEl) {
    ...
    if (svgEl instanceof SVGElement) {
      ...
      this._svgSelection = d3__namespace.select(svgEl);
    } else {
  ...
  set svgElement(svgElement) {
    ...
    this._svgSelection = d3__namespace.select(svgElement);
    this.selectionGroups = this.createWorld();
    this._zoom = createZoom.bind(this)();
  ...
  createSimulation() {
    const simulation = d3__namespace.forceSimulation().force("link", linkForce(400)).force("forceX", xForce()).force("forceY", yForce()).force("gravity", gravity(-1e4)).force("shapeCollide", shapeCollide.bind(this)).on("tick", ticked.bind(this)).on("end", () => {
  ...
  moveTo(options) {
    moveTo.bind(this)(options, (t) => {
      this.svgSelection.call(this._zoom.transform, d3__namespace.zoomIdentity.translate(t.x, t.y).scale(t.k));
    });
  }
  on(event, callback) {
...
exports.LinkStrength = LinkStrength;
exports.LinkType = LinkType;
exports.SatelliteType = SatelliteType;
